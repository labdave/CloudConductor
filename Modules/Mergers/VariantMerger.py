...
class VariantMerger(Merger):
    ...
    def define_output(self):
        ...
        merged_filt_long = self.generate_unique_file_name(extension="merged_filtered_long.txt")
        merged_filt_wide = self.generate_unique_file_name(extension="merged_filtered_wide.txt")
        merged_wl_long = self.generate_unique_file_name(extension="merged_whitelist_long.txt")
        merged_wl_wide = self.generate_unique_file_name(extension="merged_whitelist_wide.txt")
        self.add_output("merged_filt_long", merged_filt_long)
        self.add_output("merged_filt_wide", merged_filt_wide)
        self.add_ouput("merged_wl_long", merged_wl_long)
        self.add_ouput("merged_wl_wide", merged_wl_wide)
